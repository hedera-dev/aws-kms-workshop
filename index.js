...
async function transactionSigner(bytesToSign) {
  console.log(`Signing with KMS Key: ${process.env.AWS_KMS_KEY_ID}`);
...
const main = async () => {
  console.log("=== AWS KMS Signing Demo ===");
  console.log(`Using KMS Key ID: ${process.env.AWS_KMS_KEY_ID}`);
  ...
  console.log(`KMS Key ARN: ${publicResponse.KeyId}`);
  ...
  console.log(`Public Key (from KMS): ${newAccountPublicKey.toStringRaw()}`);
